...
from view.question2 import view_of_the_view, years
...
def layout():
    return html.Div([
        ...
        dbc.Card([
            ...
            dbc.CardBody([
                html.Pre(query, className="mb-0", style={'color': 'black', 'overflowY': 'scroll', 'max-height': '200px'}),
            ], style={'padding': '10px'}),
        ...
        html.Div([
            ...
            dmc.Select(placeholder="Choissez une années",
                                id="year-dropdown",
                                data=years().rename(columns={'annee_debut': 'value'}).to_dict(orient='records'),
                                value='2020'
                                ),
            html.Div(view_of_the_view('2020'), id='view-table', style={'overflowY': 'scroll', 'height': '400px'})
        ]),
        html.Br(),
        html.Br(),
        html.Br(),
...
def update_view(year):
    updated_view = view_of_the_view(year)
    return updated_view