...
def layout():
    return html.Div([
        ...
        html.Br(),
        html.H4("Quelques informations pratiques :"),
        html.P("Sur chaque page, vous trouverez le code SQL associé à différents sujets ainsi que des tables qui peuvent illustrer le résultat de la requête. Pour consulter l'ensemble des informations sur chaque page, n'hésitez pas à utiliser votre souris pour descendre sur la page et voir tout le contenu. De plus, les tables de données ont des tailles fixes pour éviter qu'elles n'occupent toute la page web. N'hésitez donc pas à utiliser la roulette de la souris pour consulter l'ensemble des données disponibles dans la table."),
        html.P("Pour accéder au code SQL lié à chaque sujet, vous le trouverez dans notre code, disponible dans le dossier 'data/sql'. Chaque fichier est nommé en fonction du sujet qu'il couvre, ce qui facilite la recherche et l'accès aux informations pertinentes."),
        html.Br(),
    ])